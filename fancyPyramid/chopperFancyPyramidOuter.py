...
def printPoly(poly,vertices,dispatcher,stepSize = 0.4, axis="x",extraExtrusion=1.0,infillExtraExtrusion=1.0,innerLines=[], reverse=True):
    print("#",poly)
    if axis == "x":
...
def sliceZLayer(polys,vertices,dispatcher,zCutoff,zHeight,axis,reverse):
    ...
    for poly in bottomConvexPolys:
        ...
        if 1==0 and (zCutoff < 0.5 or zCutoff > maxHeight-0.5):
            ...
            printPoly(poly,vertices,dispatcher,stepSize=5,axis="x",extraExtrusion=1.5,infillExtraExtrusion=2.0,innerLines=innerLines, reverse=reverse)
...
def slicePolys(polys,vertices,dispatcher): 
    ...
    while polys:
        ...
        if reverse:
            ...
            reverse = True
        sliceZLayer(polys,vertices,dispatcher,zCutoff,zHeight,axis,reverse)
        print(polys)
        zCutoff += zHeight
